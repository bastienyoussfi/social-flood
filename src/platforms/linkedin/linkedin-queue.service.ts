...
/**
 * LinkedIn Queue Event Handler
 * Listens to queue events and updates database accordingly
 */
@Processor('linkedin-posts')
export class LinkedInQueueService {
  private readonly logger = new Logger(LinkedInQueueService.name);
  ...
  async onCompleted(
    ...
    try {
      ...
      if (!platformPost) {
        this.logger.error(
          `Platform post not found for postId: ${postId} and platform: linkedin`,
        );
        return;
      }
      ...
      this.logger.log(
        `Updated platform post for LinkedIn. Post ID: ${postId}, LinkedIn ID: ${result?.platformPostId}`,
      );
    } catch (error) {
      this.logger.error(
   ...
   */
  @OnQueueFailed()
  async onFailed(job: Job<LinkedInJobData>, error: Error): Promise<void> {
    try {
