...
export class YoutubeQueueService {
  ...
  async onCompleted(
    ...
    try {
      ...
      if (!platformPost) {
        this.logger.error(
          `Platform post not found for postId: ${postId} and platform: youtube`,
        );
        return;
      }
      ...
      this.logger.log(
        `Updated platform post for YouTube. Post ID: ${postId}, YouTube ID: ${result?.platformPostId}`,
      );
    } catch (error) {
      this.logger.error(
   ...
   */
  @OnQueueFailed()
  async onFailed(job: Job<YoutubeJobData>, error: Error): Promise<void> {
    try {
