...
interface BlueskyJobResult {
  platformPostId: string;
  url: string;
}

/**
 * Bluesky Queue Event Handler
 * Listens to queue events and updates database accordingly
 */
@Processor('bluesky-posts')
export class BlueskyQueueService {
  private readonly logger = new Logger(BlueskyQueueService.name);
  ...
  @OnQueueCompleted()
  async onCompleted(
    job: Job<BlueskyJobData>,
    result: BlueskyJobResult,
  ): Promise<void> {
    try {
      ...
      if (!platformPost) {
        this.logger.error(
          `Platform post not found for postId: ${postId} and platform: bluesky`,
        );
        return;
      }
      ...
      this.logger.log(
        `Updated platform post for Bluesky. Post ID: ${postId}, Bluesky ID: ${result?.platformPostId}`,
      );
    } catch (error) {
      this.logger.error(
   ...
   */
  @OnQueueFailed()
  async onFailed(job: Job<BlueskyJobData>, error: Error): Promise<void> {
    try {
