...
export class TwitterQueueService {
  private readonly logger = new Logger(TwitterQueueService.name);
  ...
  async onCompleted(
    ...
    try {
      ...
      if (!platformPost) {
        this.logger.error(
          `Platform post not found for postId: ${postId} and platform: twitter`,
        );
        return;
      }
      ...
      this.logger.log(
        `Updated platform post for Twitter. Post ID: ${postId}, Twitter ID: ${result?.platformPostId}`,
      );
    } catch (error) {
      this.logger.error(
