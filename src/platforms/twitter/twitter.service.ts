...
/**
 * Twitter Service
 * Orchestrates the process of posting to Twitter using OAuth 2.0
 */
@Injectable()
export class TwitterService {
  ...
  /**
   * Publish a post to Twitter on behalf of a user
   * @param content - Post content with text and optional media
   * @param twitterUserId - Twitter user ID for OAuth 2.0 posting (required)
  ...
  async publishPost(
    ...
    try {
      if (!twitterUserId) {
        throw new Error(
          'Twitter user ID is required. Please authenticate with Twitter first.',
        );
      }
