...
interface PinterestJobResult {
  platformPostId: string;
  url: string;
}

/**
 * Pinterest Queue Event Handler
 * Listens to queue events and updates database accordingly
 */
@Processor('pinterest-posts')
export class PinterestQueueService {
  private readonly logger = new Logger(PinterestQueueService.name);
  ...
  @OnQueueCompleted()
  async onCompleted(
    job: Job<PinterestJobData>,
    result: PinterestJobResult,
  ): Promise<void> {
    try {
      ...
      if (!platformPost) {
        this.logger.error(
          `Platform post not found for postId: ${postId} and platform: pinterest`,
        );
        return;
      }
      ...
      this.logger.log(
        `Updated platform post for Pinterest. Post ID: ${postId}, Pinterest ID: ${result?.platformPostId}`,
      );
    } catch (error) {
      this.logger.error(
   ...
   */
  @OnQueueFailed()
  async onFailed(job: Job<PinterestJobData>, error: Error): Promise<void> {
    try {
