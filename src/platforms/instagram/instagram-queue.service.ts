...
/**
 * Instagram Queue Event Handler
 * Listens to queue events and updates database accordingly
 */
@Processor('instagram-posts')
export class InstagramQueueService {
  private readonly logger = new Logger(InstagramQueueService.name);
  ...
  async onCompleted(
    ...
    try {
      ...
      if (!platformPost) {
        this.logger.error(
          `Platform post not found for postId: ${postId} and platform: instagram`,
        );
        return;
      }
      ...
      this.logger.log(
        `Updated platform post for Instagram. Post ID: ${postId}, Instagram ID: ${result?.platformPostId}`,
      );
    } catch (error) {
      this.logger.error(
   ...
   */
  @OnQueueFailed()
  async onFailed(job: Job<InstagramJobData>, error: Error): Promise<void> {
    try {
